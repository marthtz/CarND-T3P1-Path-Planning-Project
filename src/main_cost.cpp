...
//==============================================================================
              ...
              ((next_wp - lane_change_wp) % map_waypoints_x.size()) > 3)
          {
            ...
            for (int check_lane: possible_lanes)
            {
              ...
              // lane cost
              if (check_lane != ego_lane)
                cost += 1000;

              // get distance and speed of closest car ahead
              vector<double> car_ahead = get_closest_car_ahead(check_lane, sensor_fusion, ego_car_s, prev_size);

              double distance_closest_check = car_ahead[0];
              double closest_speed = car_ahead[1];
